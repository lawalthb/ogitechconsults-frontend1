...
export const products_tb = {
	...
	mutations: {
		...
		updateRecord(state, record) {
			const item = state.records.find(item => item['product_id'] === record['product_id']);
			if(item){
		...
		deleteRecord(state, id) {
			if(Array.isArray(id)){
				id.forEach((itemId) => {
					let itemIndex = state.records.findIndex(item => item['product_id'] == itemId);
					if(itemIndex!=-1){
				...
				let itemIndex = state.records.findIndex(item => item['product_id'] == id);
				if(itemIndex!=-1){
					state.records.splice(itemIndex, 1);
				}
			}
