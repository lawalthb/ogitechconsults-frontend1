                        ...
                        <div class="col-12 col-md-auto " >
                            <div class="" >
                                <div class="row  items-center q-col-gutter-sm q-px-sm">
                                    <div class="col">
                                        <div class="text-h6 text-primary">Edit Model Has Permissions</div>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
	...
	export default {
		name: 'editModelhaspermissionsPage',
		components: {
		},
		...
		props: {
			...
			routeName: {
				type: String,
				default: 'model_has_permissionsedit',
			},
			pagePath: {
				type : String,
				default : 'model_has_permissions/edit',
			},
			apiPath: {
				type: String,
				default: 'model_has_permissions/edit',
		...
		computed: {
			pageTitle:{
				get: function () {
					return "Edit Model Has Permissions"
				}
			},
			currentRecord: {
				get: function () {
		...
		methods: {
			...mapActions('model_has_permissions', ['updateRecord', 'fetchRecord']),
				...
				if(isFormValid){
					...
					this.updateRecord(data).then(
						(response) => {
							...
							if(this.redirect) this.navigateTo(`/model_has_permissions`);
						},
						(response) => {
							this.saving = false;
			...
			resetForm (){
				...
				this.formData = {permission_id: "", model_type: "", model_id: "", };
